...
describe Van do
  ...
  describe '#deliver_bikes' do
    ...
    it 'delivers broken bikes to a garage' do
      ...
      allow(station).to receive(:release_broken_bikes)
      subject.collect_bikes(station)
  ...
  describe '#deliver_working_bikes_to_dock' do
    ...
    #   subject.collect_bikes(station)
    #   expect(subject.deliver_working_bikes_to_dock(station)).to raise_error("You can't return broken bikes to a dock")
    # end



    it 'will deliver working bikes to a station' do
      allow(station).to receive(:receive_working_bikes)
      subject.collect_working_bikes(garage)
      subject.deliver_working_bikes_to_dock(station)
      expect(subject.bikes).to be_empty
    end
